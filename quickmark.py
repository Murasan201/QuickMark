...
WATERMARK_TEXT = 'Sample Watermark'  # ウォーターマークとして挿入するテキスト
WATERMARK_POSITION = 2  # ウォーターマークの挿入位置: 1 = 画像上部, 2 = 画像中央, 3 = 画像下部
TRANSPARENCY = 50  # ウォーターマークの透明度（100 = 完全不透明, 0 = 完全透明）
SHADOW_OFFSET = 2  # テキストの影のオフセット（ピクセル単位）
SHADOW_ENABLED = True  # 影を描画するかどうかのフラグ
IMAGE_EXTENSIONS = ['.png', '.jpg', '.jpeg', '.bmp']  # 処理対象の画像ファイルの拡張子
...
def add_watermark_with_shadow(image_path, output_path):
    ...
    with Image.open(image_path).convert("RGBA") as base:
        ...
        if SHADOW_ENABLED:
            ...
            draw.text(shadow_position, WATERMARK_TEXT, font=font, fill=(0, 0, 0, int(255 * TRANSPARENCY / 100)))

        # メインのテキスト（ウォーターマーク）を描画
